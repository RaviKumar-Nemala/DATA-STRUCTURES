            ...
            for(j=0;j<=n2;j++)
            {
                ...
                else 
                {
                    ...
                    else if(p_char == '*')
                    {
                        ...
                        {
                            dp[i][j] = dp[i][j] || dp[i-1][j]; //dp[i][j-1]  is used if '.' char is replaced with the string char //dp[i][j] which is already calculted on above it used if    ".*" is ignored then we need look up on the two above"                     }
                    }
                    else if(p_char == '.')
                    {
                        dp[i][j] = dp[i-1][j-1];
